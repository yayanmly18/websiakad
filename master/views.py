...
def cetak_mahasiswa(request):
	...
	table_data.append([ "NIM", "Nama", "Alamat","Jenis Kelamin","Jenis Daftar", "Telepon", "Email", "Program Studi" ])

    #mengisi tabel
    #nama field disesuaikan dengan class Mahasiswa yang ada di dalam master
	for item in data:
		table_data.append([ item.nim, item.nama, item.alamat, item.jenis_kelamin, item.jenis_daftar, item.no_telepon, item.email, item.progdi ])
...
def list_mahasiswa(request):
	datane = Mahasiswa.objects.all()
	return render(request, "mahasiswa.html", {"data" : datane})
